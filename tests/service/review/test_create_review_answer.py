...
class TestCreateReviewAnswer(object):
    ...
    async def test_create_review_answer(
        ...
        # when
        result = await review_service.create_review_answer(
            request_user,
            dto,
            center_fixture.id,
    ...
    async def test_create_review_answer_with_not_exist_center(
        ...
        with pytest.raises(NotFoundException) as exception:
            # when
            await review_service.create_review_answer(request_user, dto, wrong_id, review_fixture.id)
    ...
    async def test_create_review_answer_with_not_center_admin(
        ...
        with pytest.raises(UnauthorizedException) as exception:
            # when
            await review_service.create_review_answer(request_user, dto, center_fixture.id, review_fixture.id)
    ...
    async def test_create_review_answer_with_not_exist_review(
        ...
        with pytest.raises(NotFoundException) as exception:
            # when
            await review_service.create_review_answer(request_user, dto, center_fixture.id, wrong_review_id)
