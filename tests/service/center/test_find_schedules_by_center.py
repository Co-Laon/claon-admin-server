...
class TestFindSchedulesByCenter(object):
    ...
    async def test_find_schedules_by_center(
            self,
        ...
        mock_repo["center_schedule"].find_by_center.return_value = [center_schedule_fixture]

        response = [ScheduleBriefResponseDto.from_entity(center_schedule_fixture)]

        # when
        result = await center_service.find_schedules_by_center(request_user, center_fixture.id)
    ...
    async def test_find_schedules_by_center_with_not_exist_center(
            self,
            center_service: CenterService,
            mock_repo: dict,
            center_fixture: Center,
        ...
        with pytest.raises(NotFoundException) as exception:
            # when
            await center_service.find_schedules_by_center(request_user, center_fixture.id)
    ...
    async def test_find_schedules_by_center_without_center_admin(
            self,
            center_service: CenterService,
            mock_repo: dict,
            center_fixture: Center,
