...
class UserService:
    ...
    @transactional()
    async def sign_up_center(self, session: AsyncSession, subject: RequestUser, req: CenterAuthRequestDto):
        ...
        if await self.user_repository.exist_by_nickname(session, req.profile.nickname):
            raise BadRequestException(
        ...
        await self.center_approved_file_repository.save_all(
            ...
            [CenterApprovedFile(user_id=subject.id, center=center, url=e)
             for e in req.proof_list]
        )

        return CenterResponseDto.from_entity(center, holds, walls)

    @transactional()
    async def sign_up_lector(self, session: AsyncSession, subject: RequestUser, req: LectorRequestDto):
        if subject.role != Role.PENDING:
            raise BadRequestException(
                ErrorCode.USER_ALREADY_SIGNED_UP,
                "이미 회원가입이 완료된 계정입니다."
        ...
        user.sign_up(**req.profile.dict())
        lector = await self.lector_repository.save(session, Lector.of(subject.id, **req.lector.dict()))
    ...
    @transactional()
    async def sign_in(self, session: AsyncSession, provider: OAuthProvider, req: SignInRequestDto):
        oauth_dto: OAuthUserInfoDto = await self.supplier.get_user_info(provider, req.id_token)
    ...
    async def upload_profile(self, file: UploadFile):
        ...
        url = await upload_file(file, "user", purpose.value)
    ...
    async def upload_file(self, purpose: LectorUploadPurpose, file: UploadFile):
        ...
        url = await upload_file(file, "lector", purpose.value)
        return UploadFileResponseDto(file_url=url)
    ...
    async def find_all_by_nickname(self, session: AsyncSession, params: Params, nickname: str):
        pages = await self.user_repository.find_all_by_nickname(session, nickname, params)

        return await paginate(UserNameResponseDto, pages)

    # TODO: Need to be removed later
    @transactional()
    async def test_sign_in(self, session: AsyncSession, req: SignInRequestDto):
        user = await self.user_repository.find_by_oauth_id(session, req.id_token)

        is_signed_up = True
        if user is None:
            is_signed_up = False
            oauth_dto = OAuthUserInfoDto(oauth_id=req.id_token, sns_email=req.id_token + "@gmail.com")
            user = await self.user_repository.save(session, User.join(**oauth_dto.dict()))
